...
OWM::OWM(const Prot p1, const Dim& d1, const Met& m1) : prot(p1), dim(d1), met(m1) {}
bool OWM::empty() const { return (prot == -1 && dim.empty() && met.empty()); }
Dmet::Dmet() : eg(0.0), seq(), pos(0) {}
Dmet::Dmet(const double E, const std::string& S, const int P) : eg(E), seq(S), pos(P) {}

...
void ToyPlugin::init_prot(std::vector<int>& perimeters, std::vector<double>& energies,
                          std::vector<std::vector<double>>& prot_prom_energies,
                          std::vector<std::vector<bool>>& prot_pol) { // reads to table the perimeter and energies
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        std::string name = cwd + "/toylife/promoters/proteins_bind_prom" + ss.str() + ".txt";
        std::ifstream in(name.c_str());
        check_file(in, name);
        int ind_prot;
        double prom_energ;
...
void ToyPlugin::init_dim(std::map<std::pair<int, int>, int>& dim_perim, std::vector<double>& dim_energy,
                         std::vector<std::vector<double>>& dim_prom_energies,
                         std::vector<std::vector<bool>>& dim_pol) { // reads to table the information relating to dimers
    int total_dimers = 0;
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        std::stringstream ss; // create a stringstream
        ss << i;              // add number to the stream
        std::string name = cwd + "/toylife/promoters/dimers_bind_prom" + ss.str() + ".txt";
        std::ifstream in(name.c_str());
        check_file(in, name);
        int ind_dim;
        double prom_energ;
...
////////////////////////////////////////////////////////////////////////
// SIMPLIFIED METS
void ToyPlugin::simplified_mets(std::vector<std::string>& mets, std::map<std::string, int>& not_broken,
...
////////////////////////////////////////////////////////////////////////
// DIM_MET
void ToyPlugin::init_dim_met(mapa_dmet& dim_met) { // key: Met; value: map -> key (Id of the dimer), value:
    ...
    for (int i = 4; i < 9; ++i) {
        ...
        std::stringstream ss;
        ss << i;
        for (std::map<int, std::vector<int>>::const_iterator it = mapa.begin(); it != mapa.end();
             ++it) { // Para todas las secuencias
            std::stringstream ss_met;
            ss_met << it->second[0];
            std::string name = cwd + "/toylife/dim_metabolism/" + ss.str() + "_" + ss_met.str() + ".txt";
...
void ToyPlugin::init_prot_met(mapa_pmet& prot_met) {
    for (int i = 4; i < 9; ++i) {
        ...
        for (std::map<int, std::vector<int>>::const_iterator it = mapa.begin(); it != mapa.end();
            ...
            ss_prot << it->second[0];
            std::string name = cwd + "/toylife/prot_metabolism/" + ss.str() + "_" + ss_prot.str() + ".txt";
...
////////////////////////////////////////////////////////////////////////
// PROMOTER_EXPRESSION
int ToyPlugin::promoter_expression(const std::vector<std::pair<int, int>>& genotype,
    ...
    for (int p = 0; p < genotype.size(); ++p) {
        ...
        for (mapa_prot::const_iterator it_prot = objects.first.begin(); it_prot != objects.first.end();
            ...
            if (d_less(prot_prom_energies[prom][it_prot->first],
                ...
                d_less(prot_prom_energies[prom][it_prot->first] + prot_energies[it_prot->first] - polymerase[prom],
                       0.0))   // it can bind it more strongly than POLY
                binding_energies.first[it_prot->first] = prot_prom_energies[prom][it_prot->first] +
        ...
        for (mapa_dim::const_iterator it_dim = objects.second.begin(); it_dim != objects.second.end();
            ...
            if (d_less(dim_prom_energies[prom][it_dim->first.id],
                ...
                d_less(dim_prom_energies[prom][it_dim->first.id] + dim_bond_energy[it_dim->first.id] +
                           prot_energies[it_dim->first.p1] + prot_energies[it_dim->first.p2] - polymerase[prom],
                       0.0)) // it can bind it more strongly than POLY
                binding_energies.second[it_dim->first] =
                    dim_prom_energies[prom][it_dim->first.id] + dim_bond_energy[it_dim->first.id] +
                    prot_energies[it_dim->first.p1] + prot_energies[it_dim->first.p2] -
                    polymerase[prom]; // we substract the binding energy of the
                                      // polymerase; if it is greater than
                                      // zero, binding energy is zero
        ...
        while (1) {
            ...
            for (std::map<Prot, double>::const_iterator it1 = binding_energies.first.begin();
                 it1 != binding_energies.first.end(); ++it1) { // loop over all proteins, we save its id (i_min) and
                                                               // its energy (min)
            ...
            for (std::map<Dim, double>::const_iterator it2 = binding_energies.second.begin();
                 it2 != binding_energies.second.end(); ++it2) { // loop over all dimers, we save its id (i_min) and
                                                                // its energy (min)
            ...
            if (!i_min) {                                         // if the minimum is a protein
                for (std::map<Prot, double>::const_iterator it1 = std::next(prot_min);
                     it1 != binding_energies.first.end(); ++it1)  // we start the loop on the object following the
                ...
                for (std::map<Dim, double>::const_iterator it2 = binding_energies.second.begin();
                     it2 != binding_energies.second.end(); ++it2) // we compare the minimum protein with the dimers
                   ...
                   // first)
                for (std::map<Dim, double>::const_iterator it2 = std::next(dim_min);
                     it2 != binding_energies.second.end(); ++it2) // we start the loop on the object following the
                                                                  // minimal (the others have greater binding
                                                                  // energies)
            ...
            if (count_mins) { // if there are repeated energies
                for (std::map<Prot, double>::iterator it1 = binding_energies.first.begin();
                     it1 != binding_energies.first.end(); ++it1)
                    if (d_equal(it1->second,
                                min))                      // if the binding energies are the same
                        binding_energies.first.erase(it1); // we sum to the counter
                for (std::map<Dim, double>::iterator it2 = binding_energies.second.begin();
    ...
    for (int i = 0; i < promoter_active.size(); ++i)
        if (promoter_active[i])
            objects.first[prot_gen[genotype[i].second]]++; // we add the proteins
                                                           // that are expressed
...
////////////////////////////////////////////////////////////////////////
// FINDING_CYCLES
bool ToyPlugin::finding_cycles(std::vector<std::pair<int, std::pair<mapa_prot, mapa_dim>>>& iterations,
                               int expression) {
    // OUTPUT
    ...
    for (int i = 0; i != iterations.size(); ++i)
        if (iterations[i].first == expression) {      // if, at any point, this state has been visited
...
void ToyPlugin::dimerization(std::pair<mapa_prot, mapa_dim>& objects) {
    // WHICH DIMERS FORM
    std::map<double, mapa_dim> binding_energies; // creates a map of energies, so that dimers with the
                                                 ...
                                                 // formed
    for (mapa_prot::const_iterator it1 = objects.first.begin(); it1 != objects.first.end();
        ...
        for (mapa_prot::const_iterator it2 = it1; it2 != objects.first.end();
            ...
            if (it1 != it2 || (it1 == it2 && it1->second > 1)) { // Prots will only dimerize with themselves if there
                ...
                if (it_dim != dim_perim.end()) { // if this pair forms a dimer,
                                                 // we form the tuple
                    double b_energy =
                        dim_bond_energy[it_dim->second] + prot_energies[it1->first] + prot_energies[it2->first];
                    binding_energies[b_energy][Dim(it_dim->second, it1->first, it2->first)] +=
    ...
    for (std::map<double, mapa_dim>::iterator it = binding_energies.begin(); it != binding_energies.end(); ++it) {
        std::map<Prot, int> number; // will hold how many complexes each protein is involved in
                                    // (if it's greater than objects.first[i], then no dimers
                                    // is formed
        for (mapa_dim::iterator it_dim = it->second.begin(); it_dim != it->second.end();
             ++it_dim) {            // for each possible dimer
        ...
        for (mapa_dim::iterator it_dim = it->second.begin(); it_dim != it->second.end(); ++it_dim) // for each dimer
            if (objects.first[it_dim->first.p1] < number[it_dim->first.p1] || // if either of the two prots
                                                                              // take part
...
////////////////////////////////////////////////////////////////////////
// BOOLEAN NETWORK
std::vector<std::pair<int, std::pair<mapa_prot, mapa_dim>>> ToyPlugin::boolean_network(
    const std::vector<std::pair<int, int>>& genotype) {
    // COMPUTES THE ATTRACTOR OF THE DYNAMICS OF A toyLIFE ORGANISM
    std::pair<mapa_prot, mapa_dim> objects;
    std::vector<std::pair<int, std::pair<mapa_prot, mapa_dim>>> cycle(1,
                                                                      std::make_pair(0, objects)); // first state is 0
    ...
    while (1) {
        ...
        dimerization(objects);                    // dimerize
        cycle.push_back(std::make_pair(new_state,
...
////////////////////////////////////////////////////////////////////////
// REACTING
void ToyPlugin::reacting(std::pair<mapa_prot, mapa_dim>& objects, mapa_met& met, mapa_owm& objects_with_mets,
                         double& fit) {
    // GIVEN A SET OF OBJECTS, IT TELLS US WHAT BINDS WHAT AND WHAT BREAKS WHAT
    std::map<double, mapa_owm> binding_energies; // creates a map of energies, so that complexes with
    ...
    for (mapa_prot::const_iterator it1 = objects.first.begin(); it1 != objects.first.end();
        ...
        for (mapa_prot::const_iterator it2 = it1; it2 != objects.first.end();
            ...
            if (it1 != it2 || (it1 == it2 && it1->second > 1)) { // Prots will only dimerize with themselves if there
                ...
                if (it_dim != dim_perim.end()) {                 // if this pair forms a dimer,
                                                                 // we form the tuple
                    double b_energy = dim_bond_energy[it_dim->second] + prot_energies[it1->first] +
        ...
        for (mapa_met::const_iterator it_met = met.begin(); it_met != met.end(); ++it_met) {
            ...
            try { // look and see if the dimer binds to the met
                ...
                if (d_less(tupla.eg,
                    ...
                    double b_energy = tupla.eg + dim_bond_energy[it_dim->first.id] + prot_energies[it_dim->first.p1] +
    ...
    for (mapa_owm::const_iterator it_dm = objects_with_mets.begin(); it_dm != objects_with_mets.end(); ++it_dm) {
        ...
        for (mapa_prot::const_iterator it_prot = objects.first.begin(); it_prot != objects.first.end(); ++it_prot) {
            ...
            try {         // look and see if the protein can break this dimer
                ...
                if (c_min < 2) // the sequences should be ordered in such a way that
                               // the first half is always the first protein
                    b_energy += prot_energies[it_dm->first.dim.p1]; // sum energy of
                                                                    // first subunit
                ...
                //   of first subunit
                    ...
                    b_energy += prot_energies[it_dm->first.dim.p2];     // sum energy of
                                                                        // second subunit
                double met_energy = dim_bond_energy[it_dm->first.dim.id] + prot_energies[it_dm->first.dim.p1] +
    ...
    for (std::map<double, mapa_owm>::iterator it = binding_energies.begin(); it != binding_energies.end(); ++it) {
        ...
        for (mapa_owm::iterator it_tuple = it->second.begin(); it_tuple != it->second.end();
             ++it_tuple)                       // for each complex
        ...
        for (mapa_owm::iterator it_tuple = it->second.begin(); it_tuple != it->second.end();
             ++it_tuple)                       // for each complex
            if (it_tuple->first.met.empty()) { // protein-protein complex
                if (objects.first[it_tuple->first.dim.p1] < number_prots[it_tuple->first.dim.p1] || // prot1 more
                                                                                                    // involved
                    objects.first[it_tuple->first.dim.p2] < number_prots[it_tuple->first.dim.p2])   // prot2 more
                                                                                                    // involved
                ...
                if (it_tuple->first.dim.empty()) {   // protein-met complex
                    if (objects.first[it_tuple->first.prot] < number_prots[it_tuple->first.prot] || // proteins
                                                                                                    // more
                                                                                                    // involved
                        met[it_tuple->first.met] < number_mets[it_tuple->first.met])                // met more
                                                                                                    // involved
                    ...
                    if (objects.first[it_tuple->first.prot] <
                            number_prots[it_tuple->first.prot] ||            // protein more involved
                        objects_with_mets[new_tuple] < number_dm[new_tuple]) // d+m more involved
                        it->second.erase(it_tuple);                          // this D+M will not be broken
                }
            } // proteins and mets are involved
            else {                                                                            // dimer-met complex
                if (objects.second[it_tuple->first.dim] < number_dims[it_tuple->first.dim] || // dimer more
                                                                                              // involved
        ...
        for (mapa_owm::const_iterator it_tuple = it->second.begin(); it_tuple != it->second.end();
             ++it_tuple)                                                   // for each complex
            if (it_tuple->first.met.empty()) {                             // protein-protein complex
                ...
                if (it_tuple->first.dim.empty()) {                         // protein-met complex
                    ...
                    Dmet tupla         = dim_met.at(par1);
                    std::string pmet1  = it_tuple->first.met.substr(0, tupla.pos); // we split the met in the two parts
...
////////////////////////////////////////////////////////////////////////
// METABOLISM
int ToyPlugin::metabolism(const std::vector<std::pair<int, int>>& genotype, std::pair<mapa_prot, mapa_dim> objects,
                          mapa_met& met) {
    ...
    while (1) {     // we loop over the cycle until the metabolite is broken or
        ...
        ++cuenta;
        if (cuenta > int_pow(2, gene_number)) // if it hasn't broken the Met in 2^n
...
////////////////////////////////////////////////////////////////////////
// PHENOTYPE
std::vector<int> ToyPlugin::vec_phenotype(const std::vector<std::pair<int, int>>& genotype,
...
////////////////////////////////////////////////////////////////////////
// GENOTYPE_STR_TO_VEC
std::vector<std::pair<int, int>> ToyPlugin::genotype_str_to_vec(const std::string& genotype) const {
...
double ToyPlugin::metabolic_robustness(const std::string& genotype, const std::vector<int>& phen,
                                       const std::vector<std::string>& env) {
    double rob  = 0.0;
...
std::map<std::vector<int>, int> ToyPlugin::metabolic_evolvability(const std::string& genotype,
...
std::map<std::vector<int>, int> ToyPlugin::regulatory_evolvability(const std::string& genotype,

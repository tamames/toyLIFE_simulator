...
double union_seq(int side1, int side2) {
    ...
    for (int i = 0; i < side_1.size(); ++i) {
        if ((side_1[i] == '0' && side_2[3 - i] == '1') ||
            (side_1[i] == '1' && side_2[3 - i] == '0'))
            energy -= 0.3;
...
double union_seq3(int side1, const std::string& met) {
    // OUTPUT
    // Given one side and a metabolite, UNION_SEQ3 returns the minimal binding
    // energy
    ...
    std::vector<double> energy(
        met.size() - 3, 0.0);  // in a metabolite of size 8 there are 5 sides
    ...
    for (int i = 0; i < energy.size(); ++i) {
        energy[i] = union_seq(
            side1,
            bintodec(met.substr(i, 4)));  // we check every possible combination
...
struct HammingFunc {
    inline int operator()(char s1, char s2) { return s1 == s2 ? 0 : 1; }
};

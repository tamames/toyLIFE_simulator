...
Agent::Agent(float energy, std::string genotype, int parent, int sizeGenotype, mapa_prot prots, mapa_dim dims,
             mapa_met mets) {
    if (genotype.empty()) {
        this->genotype = binaryGenerator(sizeGenotype, 0.5f);
    } else {
        if (genotype.size() != sizeGenotype) // to prevent errors
            throw std::invalid_argument("The genotype must be of size " + std::to_string(sizeGenotype) + ".");
...
void Agent::print(bool complete) {
    std::cout << "ID: " << id << ". Energy: " << energy << ". Age: " << age << ". Parent: " << parent << std::endl;
...
bool Agent::checkPDM() { return (prots.empty() && dims.empty() && mets.empty()); }
...
void Agent::promoter_expression(const ToyPlugin& toy) {
    ...
    // VEC GENOTYPE
    auto vgenotype = toy.genotype_str_to_vec(genotype); // transforms binary string into list of promoters and genes
    ...
    for (int p = 0; p < vgenotype.size(); ++p) {
        ...
        for (mapa_prot::const_iterator it_prot = prots.begin(); it_prot != prots.end();
             ++it_prot)        // for every protein in PROTS
            if (d_less(toy.prot_prom_energies[prom][it_prot->first],
                       0.0) && // if the protein can bind to PROM AND
                d_less(toy.prot_prom_energies[prom][it_prot->first] + toy.prot_energies[it_prot->first] -
                           toy.polymerase[prom],
                       0.0)) // it can bind it more strongly than POLY
                binding_energies.first[it_prot->first] = toy.prot_prom_energies[prom][it_prot->first] +
        ...
        for (mapa_dim::const_iterator it_dim = dims.begin(); it_dim != dims.end(); ++it_dim) // for every dimer in DIMS
            if (d_less(toy.dim_prom_energies[prom][it_dim->first.id],
                       0.0) && // if the dimer can bind to PROM AND
                d_less(toy.dim_prom_energies[prom][it_dim->first.id] + toy.dim_bond_energy[it_dim->first.id] +
                           toy.prot_energies[it_dim->first.p1] + toy.prot_energies[it_dim->first.p2] -
                           toy.polymerase[prom],
                       0.0)) // it can bind it more strongly than POLY
                binding_energies.second[it_dim->first] =
                    toy.dim_prom_energies[prom][it_dim->first.id] + toy.dim_bond_energy[it_dim->first.id] +
                    toy.prot_energies[it_dim->first.p1] + toy.prot_energies[it_dim->first.p2] -
                    toy.polymerase[prom]; // we subtract the binding energy of
                                          // the polymerase; if it is greater
                                          // than zero, binding energy is zero
            ...
            if (count_mins) { // if there are repeated energies
                ...
                for (auto it2 = binding_energies.second.begin(); it2 != binding_energies.second.end();) {
                    if (d_equal(it2->second, min))
                     ...
                     // activated
                if (!i_min)
                    promoter_active[p] = toy.prot_pol[prom][prot_min->first];  // return the minimum
                else
                    promoter_active[p] = toy.dim_pol[prom][dim_min->first.id]; // return the minimum
                break;                                                         // we exit the loop
            }
        } // while TRUE
    } // for promoters
    ...
    for (int i = 0; i < promoter_active.size(); ++i) {
        if (promoter_active[i]) {
            prots[toy.prot_gen[vgenotype[i].second]]++; // we add the proteins
                                                        // that are expressed
            energy -= TRANSLATION_ENERGY;               // we subtract the energy from translation
        }
    }
...
void Agent::reacting(const ToyPlugin& toy) {
    ...
    for (mapa_prot::const_iterator it1 = prots.begin(); it1 != prots.end();
        ...
        for (mapa_prot::const_iterator it2 = it1; it2 != prots.end();
            ...
            if (it1 != it2 || (it1 == it2 && it1->second > 1)) { // Prots will only dimerize with themselves if
                                                                 // there is more than one copy
                std::pair<int, int> perim =
                    std::make_pair(toy.prot_perimeters[it1->first], toy.prot_perimeters[it2->first]);
                std::map<std::pair<int, int>, int>::const_iterator it_dim = toy.dim_perim.find(perim);
                if (it_dim != toy.dim_perim.end()) {                 // if this pair forms a dimer,
                                                                     // we form the tuple
                    double b_energy = toy.dim_bond_energy[it_dim->second] + toy.prot_energies[it1->first] +
    ...
    for (mapa_prot::const_iterator it_prot = prots.begin(); it_prot != prots.end(); ++it_prot)
        for (mapa_met::const_iterator it_met = mets.begin(); it_met != mets.end(); ++it_met) {
            ...
            try { // see if the prot binds the met
                ...
                if (d_less(e1,
                    ...
                    double b_energy = e1 + toy.prot_energies[it_prot->first]; // energy of
                                                                              // the complex
    ...
    for (mapa_dim::const_iterator it_dim = dims.begin(); it_dim != dims.end(); ++it_dim)
        for (mapa_met::const_iterator it_met = mets.begin(); it_met != mets.end(); ++it_met) {
            ...
            try { // look and see if the dimer binds to the met
                ...
                if (d_less(tupla.eg,
                    ...
                    double b_energy = tupla.eg + toy.dim_bond_energy[it_dim->first.id] +
    ...
    for (mapa_owm::const_iterator it_dm = owns.begin(); it_dm != owns.end(); ++it_dm) {
        ...
        for (mapa_prot::const_iterator it_prot = prots.begin(); it_prot != prots.end(); ++it_prot) {
            ...
            try {         // look and see if the protein can break this dimer
                double b_energy = toy.prot_breaking.at(par2).first + toy.prot_energies[it_prot->first];
                int c_min       = toy.prot_breaking.at(par2).second;
                if (c_min < 2) // the sequences should be ordered in such a way
                               // that the first half is always the first protein
                    b_energy += toy.prot_energies[it_dm->first.dim.p1]; // sum energy
                                                                        // of first
                                                                        // subunit
                ...
                //   energy of first subunit
                    ...
                    b_energy += toy.prot_energies[it_dm->first.dim.p2];     // sum energy
                                                                            // of second
                                                                            // subunit
                double met_energy = toy.dim_bond_energy[it_dm->first.dim.id] + toy.prot_energies[it_dm->first.dim.p1] +
    ...
    for (auto it = binding_energies.begin(); it != binding_energies.end(); ++it) {
        ...
        for (mapa_owm::iterator it_tuple = it->second.begin(); it_tuple != it->second.end();) { // for each complex
            if (it_tuple->first.met.empty()) {                                              // protein-protein complex
                if (prots[it_tuple->first.dim.p1] < number_prots[it_tuple->first.dim.p1] || // prot1 more
                                                                                            // involved
                    prots[it_tuple->first.dim.p2] < number_prots[it_tuple->first.dim.p2]) { // prot2 more
                                                                                            // involved
                ...
                if (it_tuple->first.dim.empty()) {   // protein-met complex
                    if (prots[it_tuple->first.prot] < number_prots[it_tuple->first.prot] || // proteins
                                                                                            // more
                                                                                            // involved
                        mets[it_tuple->first.met] < number_mets[it_tuple->first.met])       // met more
                                                                                            // involved
                ...
                if (dims[it_tuple->first.dim] < number_dims[it_tuple->first.dim] || // dimer more
                                                                                    // involved
...
std::pair<Agent, Agent> divide(Agent parent) {
    ...
    if (!parent.prots.empty()) {
        std::pair<mapa_prot, mapa_prot> child_prots = mapDistribution(parent.prots);
...
Population::Population(int sizePopulation) {
    ...
    std::vector<float> energies = createRandomArray(sizePopulation, ENERGY_TO_REPRODUCE * 1.25);
...
std::vector<std::map<std::string, int>> Population::iteration(std::map<std::string, int>& foodMap, const ToyPlugin& toy,
        ...
        if (print) {
            if (tenPercent == 0 & i + 1 == sizePopulation) {
                std::cout << currentTime() << "   In iteration " << iterationNumber
                          << " . Progress:" << i / tenPercent * 10 << "%  Processed Agents: " << i + 1 << "\n";
    ...
    averageEnergyGain.push_back(static_cast<double>(sum) / (float)sizePopulation);
...
void Population::afterIteration(bool print) {
    ...
    if (!reproducePositions.empty()) {
        ...
        for (int i = 0; i < reproducePositions.size(); ++i) {
            std::pair<Agent, Agent> children = divide(agents[reproducePositions[i]]);
...
void Population::deleteElements(std::vector<Agent>& agents, const std::vector<int>& indexes) {
...
std::vector<Agent>::iterator Population::eraseFast(std::vector<Agent>& c, std::vector<Agent>::iterator it) {
    if (&(*it) == &(c.back())) {
        c.pop_back();
        return std::end(c);
    }
...
std::vector<std::string> Population::getPopulationEnergy() {
    ...
    results[1] = std::to_string(static_cast<double>(sum) / (float)sizePopulation);
...
std::vector<std::vector<std::string>> Population::getPopulationGains() {
    ...
    for (size_t i = 0; i < maxEnergyGain.size(); ++i) {
        gainedEnergy[i] = {std::to_string(maxEnergyGain[i]), std::to_string(averageEnergyGain[i]),
                           std::to_string(totalEnergyGain[i]), std::to_string(minEnergyGain[i])};
    }
    return gainedEnergy;
}
...
std::vector<std::vector<std::string>> Population::getDeadsAndReproduces() {
    ...
    for (size_t i = 0; i < numberOfDeads.size(); ++i) {
        deadsReproduce[i] = {std::to_string(numberOfDeads[i]), std::to_string(numberOfReproductions[i])};
    }

...
int main() {
    ...
    for (int i = 1; i <= numberOfGenerations; ++i) {
        ...
        std::vector<std::map<std::string, int>> returnedFood = population.iteration(food2eatMap, toy, print, i);
        ...
        if (population.sizePopulation == 0) {
            std::cout << "\n All the population died in iteration: " << i << std::endl;
            break;
        }
        ...
        if (i % tenPercent == 0) {
            std::cout << currentTime() << ":   Iteration: " << i << ". Size: " << population.sizePopulation
                      << std::endl;
        }

        if (population.sizePopulation > MAXIMUM_POPULATION_SIZE) {
            std::cout << "The population has exceeded the limit of " << MAXIMUM_POPULATION_SIZE
                      << " in iteration: " << i << std::endl;
            break;
        }

        finishTime = (double)clock();
        std::cout << "Time of iteration " << i << ": " << (finishTime - startTime) / CLOCKS_PER_SEC
                  << ". Size: " << population.sizePopulation << std::endl;
    }

    std::cout << currentTime() << "    End of the simulation\n" << std::endl;

    std::vector<std::vector<std::string>> gainedEnergies = population.getPopulationGains();

    writeResults("energy_gains", dataDirectory, {"Max_Gain", "Average_Gain", "Total_Gain", "Min_Gain"}, gainedEnergies);

    std::vector<std::vector<std::string>> deadsReproduce = population.getDeadsAndReproduces();

    writeResults("deads_reproduce", dataDirectory, {"Deads", "Reproduce"}, deadsReproduce);

    writeResults("energies", dataDirectory, {"Max", "Average", "Min"}, {energyInfo});
